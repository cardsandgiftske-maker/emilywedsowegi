...
export const RsvpSection: React.FC = () => {
  ...
  const handleStartEdit = () => {
    if (submittedRsvp) {
      setFormData({
        ...
        guestCount: Math.min(submittedRsvp.guestCount || 1, 3),
      });
    }
    setIsEditing(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const rsvpPayload = {
        ...
        guestCount: formData.attending === 'yes' ? Math.min(formData.guestCount, 3) : 0,
        dietary: null,
        message: null,
      };
  ...
  const whatsappContactUrl = `https://wa.me/${WEDDING_DETAILS.rsvpPhoneInternational.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(
    "Hello! I would like to inquire regarding Emilly & Owegi's Nyombo Ceremony on October 10, 2026."
  )}`;
        ...
        {submittedRsvp && !isEditing ? (
          <div className="max-w-lg mx-auto bg-[#F5EFE6] border border-[#D8C7B5] rounded-2xl p-6 shadow-md text-center space-y-4">
            ...
            <div className="p-4 bg-[#FAF6EE] rounded-xl border border-[#D8C7B5] text-left space-y-2 text-xs font-sans">
              <div className="flex justify-between border-b border-gray-200 pb-2">
                ...
                <span className={`font-bold ${submittedRsvp.attending === 'yes' ? 'text-[#2B1E1A]' : 'text-[#C85A32]'}`}>
                  {submittedRsvp.attending === 'yes' ? 'Joyfully Attending 🎉' : 'No, Sends Love ❤️'}
                </span>
              </div>
          ...
          <form onSubmit={handleSubmit} className="max-w-xl mx-auto space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                <span className="block text-base">No, Sends Love ❤️</span>
                <span className="text-[11px] font-normal opacity-80">Sending blessings &amp; love</span>
              </button>
            </div>
            ...
            {/* Number of Guests if attending (Max 3) */}
            {formData.attending === 'yes' && (
              <div>
                <label className="block text-xs font-sans uppercase font-bold text-[#2B1E1A] mb-1">
                  Number of Guests <span className="text-gray-500 font-normal text-[11px]">(Maximum 3)</span>
                </label>
                <select
