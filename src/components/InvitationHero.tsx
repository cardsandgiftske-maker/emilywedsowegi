    ...
    <section id="invitation" className="relative pt-0 pb-12 md:pb-16 w-full px-0">
      ...
      <div className="relative z-10 px-4 sm:px-12 md:px-16 pt-8 sm:pt-12 pb-14 sm:pb-20 max-w-2xl mx-auto text-center">
        ...
        <div className="space-y-4 sm:space-y-6 relative z-10 max-w-xl mx-auto">
          ...
          <div className="flex flex-col items-center justify-center mb-2 sm:mb-4 space-y-4">
            {/* E&O Monogram Badge */}
            <div className="relative flex items-center justify-center">
              <div className="w-28 h-28 sm:w-36 sm:h-36 rounded-full bg-[#2B1E1A] border-3 sm:border-4 border-[#D4A359] shadow-xl flex flex-col items-center justify-center relative overflow-hidden group">
                <div className="absolute inset-1.5 sm:inset-2.5 rounded-full border border-dashed border-[#D4A359]/70 pointer-events-none" />
                <div className="flex items-center justify-center gap-1.5 font-serif text-2xl sm:text-4xl font-bold text-[#F5EFE6] tracking-widest pt-1">
                  <span>E</span>
                  <span className="text-[#C85A32] text-xl sm:text-3xl font-light">&amp;</span>
                  <span>O</span>
                </div>
                <div className="text-[10px] sm:text-xs font-sans tracking-[0.3em] text-[#D4A359] uppercase mt-0.5 sm:mt-1 font-bold">
                  CEREMONY
                </div>
              </div>
            </div>

            <UploadedTribalDiamondSymbolSVG size={180} className="drop-shadow-2xs" />
          </div>

          {/* Subheading Eyebrow */}
          <p className="text-[11px] sm:text-xs font-serif tracking-[0.25em] uppercase text-[#C85A32] font-bold">
            TOGETHER WITH THEIR FAMILIES,
          </p>

          {/* Couple's Names Calligraphy - DOMINANT CENTERPIECE */}
          ...
          {/* 4. Date & Location Grid (WHEN & WHERE) */}
          ...
          {/* 5. Featured Scripture Blessing Card (Below WHEN & WHERE) */}
          ...
          {/* 7. Bottom Quick Action Controls: Calendar, Music, View Digital Card & Share */}
          <div className="pt-4 border-t border-[#D8C7B5] flex items-center justify-center gap-3 relative z-30">
            <a
              href={googleCalendarUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="px-3.5 py-2 rounded-full bg-[#FAF6EE] hover:bg-[#F0E8DC] text-[#2B1E1A] border border-[#D8C7B5] transition-all active:scale-95 flex items-center gap-1.5 text-xs font-semibold shadow-2xs"
            ...
            <button
              ...
              className="px-3.5 py-2 rounded-full bg-[#FAF6EE] hover:bg-[#F0E8DC] text-[#2B1E1A] border border-[#D8C7B5] transition-all active:scale-95 flex items-center gap-1.5 text-xs font-semibold shadow-2xs cursor-pointer"
              title={isPlayingAudio ? "Mute Background Music" : "Play Background Music"}
            >
              {isPlayingAudio ? (
                <>
                  <Volume2 className="w-4 h-4 text-[#C85A32] animate-pulse" />
                  <span className="hidden sm:inline text-[#C85A32]">Music On</span>
                </>
              ) : (
                <>
            ...
            <button
              ...
              className="px-3.5 py-2 rounded-full bg-[#FAF6EE] hover:bg-[#F0E8DC] text-[#2B1E1A] border border-[#D8C7B5] transition-all active:scale-95 flex items-center gap-1.5 text-xs font-semibold shadow-2xs cursor-pointer"
              title="View Full Invitation Card"
            >
              <ImageIcon className="w-4 h-4 text-[#C85A32]" />
              <span className="hidden sm:inline">Digital Card</span>
            </button>
