...
import { ShieldCheck, Lock, X, RefreshCw, Download, Trash2, Search, Users, CheckCircle, XCircle, AlertCircle, Eye, EyeOff, MessageSquare, Heart } from 'lucide-react';
import { 
  getAllRsvpsFromFirestore, 
  deleteRsvpFromFirestore, 
  RsvpData,
  getAllGuestbookFromFirestore,
  deleteGuestbookFromFirestore,
  GuestbookData
} from '../lib/firebase';
...
export const AdminModal: React.FC<AdminModalProps> = ({ isOpen, onClose }) => {
  ...
  // Guestbook state
  const [guestbookMessages, setGuestbookMessages] = useState<Array<GuestbookData & { id: string }>>([]);
  const [guestbookSearch, setGuestbookSearch] = useState('');
  const [isGbLoading, setIsGbLoading] = useState(false);
  const [deletingGbId, setDeletingGbId] = useState<string | null>(null);
  ...
  const handleDeleteGuestbook = async (id: string, name: string) => {
    if (window.confirm(`Are you sure you want to delete the guestbook message from "${name}"?`)) {
      setDeletingGbId(id);
      try {
        await deleteGuestbookFromFirestore(id);
        setGuestbookMessages(prev => prev.filter(g => g.id !== id));
      } catch (err) {
        console.error('Failed to delete guestbook message:', err);
        alert('Failed to delete message.');
      } finally {
        setDeletingGbId(null);
      }
    }
  };
  ...
  const filteredGuestbook = guestbookMessages.filter(g => {
    return g.name.toLowerCase().includes(guestbookSearch.toLowerCase()) ||
      g.message.toLowerCase().includes(guestbookSearch.toLowerCase()) ||
      g.relationship.toLowerCase().includes(guestbookSearch.toLowerCase());
  });
      ...
      <div className="bg-[#FAF6EE] w-full max-w-4xl rounded-2xl border-2 border-[#D4A359] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="bg-[#2B1E1A] text-white px-6 py-4 border-b border-[#D4A359]/60 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-serif font-bold text-lg text-[#F5EFE6]">
              Emilly &amp; Owegi — Ceremony Admin Panel
            </h3>
          </div>
        ...
        {!isAuthenticated ? (
          ...
          <div className="p-8 sm:p-12 text-center max-w-md mx-auto space-y-6 my-auto">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-[#2B1E1A]/80 font-sans">
                Please enter the passcode to access RSVP attendance records and guestbook messages.
              </p>
            </div>
          ...
          <div className="p-4 sm:p-6 space-y-5 overflow-y-auto flex-1">
            ...
            {/* Tab Navigation */}
            <div className="flex items-center gap-2 border-b border-[#D8C7B5] pb-3">
              <button
                onClick={() => setActiveTab('rsvps')}
                className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 cursor-pointer ${
                  activeTab === 'rsvps'
                    ? 'bg-[#2B1E1A] text-[#D4A359] shadow-sm'
                    : 'bg-white text-[#2B1E1A] hover:bg-[#F5EFE6] border border-[#D8C7B5]'
                }`}
              >
                <Users className="w-4 h-4" />
                <span>RSVP Attendance ({rsvps.length})</span>
              </button>
              ...
              >
                ...
                <span>Guestbook Messages ({guestbookMessages.length})</span>
              </button>
            </div>
            ...
            {activeTab === 'rsvps' ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  ...
                  <div className="bg-[#C85A32]/15 border border-[#C85A32]/40 p-4 rounded-xl shadow-sm text-[#2B1E1A]">
                    <div className="text-[11px] text-[#C85A32] uppercase tracking-wider font-bold flex items-center gap-1">
                      <XCircle className="w-3.5 h-3.5 text-[#C85A32]" /> Sends Love
                    </div>
                          ...
                          <tr key={rsvp.id} className="hover:bg-amber-50/40 transition-colors">
                            ...
                            <td className="py-3.5 px-4 text-center whitespace-nowrap">
                              {rsvp.attending === 'yes' ? (
                                ...
                                <span className="inline-flex items-center gap-1 bg-amber-100 text-[#C85A32] text-[10px] font-bold px-2 py-0.5 rounded-full border border-amber-300">
                                  <XCircle className="w-3 h-3 text-[#C85A32]" /> Sends Love
                                </span>
                              )}
                            </td>
              ...
              <div className="space-y-4">
                {/* Search & Refresh */}
                <div className="flex items-center justify-between gap-3 bg-[#F5EFE6] p-3.5 rounded-xl border border-[#D8C7B5]">
                  <div className="relative flex-1">
                    <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" />
                    <input
                      type="text"
                      placeholder="Search messages by name, relationship, or text..."
                  ...
                  <button
                    ...
                    disabled={isGbLoading}
                    className="p-2 bg-white hover:bg-gray-100 text-[#2B1E1A] border border-[#D8C7B5] rounded-lg transition-all cursor-pointer flex items-center gap-1.5 text-xs font-semibold"
                    title="Refresh Guestbook"
                  >
                    <RefreshCw className={`w-4 h-4 ${isGbLoading ? 'animate-spin' : ''}`} />
                    <span>Refresh</span>
                  </button>
                </div>
                ...
                {isGbLoading ? (
                  <div className="py-12 text-center text-[#2B1E1A]">
                    <RefreshCw className="w-6 h-6 animate-spin mx-auto text-[#C85A32] mb-2" />
                    <p className="text-xs font-sans">Loading Guestbook blessings from Firestore...</p>
                  </div>
                ) : filteredGuestbook.length === 0 ? (
                  <div className="py-12 text-center bg-white rounded-xl border border-dashed border-[#D8C7B5] p-6 text-[#2B1E1A]">
                    <p className="font-serif font-bold text-sm text-[#2B1E1A] mb-1">No guestbook messages found</p>
                    <p className="text-xs">
                      {guestbookSearch ? 'No messages match your search query.' : 'No celebratory messages have been posted yet.'}
                    </p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {filteredGuestbook.map((entry) => (
                      <div key={entry.id} className="bg-white border border-[#D8C7B5] rounded-xl p-4 shadow-xs space-y-2 relative group">
                        <div className="flex items-center justify-between pr-8">
                          <div>
                            <h5 className="font-serif font-bold text-sm text-[#2B1E1A]">{entry.name}</h5>
                            <span className="text-[10px] text-[#C85A32] font-semibold uppercase">{entry.relationship}</span>
                          </div>
                        ...
                        <p className="text-xs text-[#2B1E1A]/90 italic font-sans leading-relaxed">
                          &ldquo;{entry.message}&rdquo;
                        </p>

                        <button
                          onClick={() => handleDeleteGuestbook(entry.id, entry.name)}
                          disabled={deletingGbId === entry.id}
                          className="absolute top-3 right-3 p-1.5 bg-red-50 hover:bg-red-100 text-red-600 rounded border border-red-200 transition-colors cursor-pointer opacity-70 hover:opacity-100"
